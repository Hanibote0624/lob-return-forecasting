...
Notes:
- Handles invalid mids using prefix sums of (mid, valid_mask).
- Requires finite, nondecreasing timestamps. Rows are never reordered because
  Stage3 must remain aligned with the source CSV.
"""

try:
    from .artifact_contract import sha256_file
    from .target_contract import label_definition, json_sha256, target_spec, binding_from_stats, require_raw_definition
except ImportError:
    from artifact_contract import sha256_file
    from target_contract import label_definition, json_sha256, target_spec, binding_from_stats, require_raw_definition
...
def compute_mid(bid1: np.ndarray, ask1: np.ndarray, lastp: np.ndarray, prefer_bidask: bool = True) -> np.ndarray:
    ...
    If prefer_bidask:
      - use (bid1+ask1)/2 where bid/ask are finite, positive, and not crossed
    ...
    # Preserve half-tick midpoints. Rounding here changes the return target.
    return mid.astype(np.float64, copy=False)
...
def worker_build_raw(task: Task) -> dict:
    ...
    # Label construction uses searchsorted and therefore always requires a
    # finite, nondecreasing event-time sequence. We never reorder here because
    # Stage3 must retain exact row alignment with the source CSV.
    if (not np.all(np.isfinite(t_sec))) or np.any(np.diff(t_sec) < 0):
        return {"ok": False, "path": task.path, "error": "timestamp_not_monotonic_or_nan"}

    # mid
    mid = compute_mid(bid1, ask1, lastp, prefer_bidask=prefer_bidask)
    label_cfg = cfg.get("label", {}) or {}
    aggregation = str(label_cfg.get("future_aggregation", "event_mean"))
    if aggregation != "event_mean":
        return {"ok": False, "path": task.path, "error": f"unsupported_future_aggregation: {aggregation}"}

    labels = build_event_mean_return_labels(
        t_sec=t_sec,
        mid=mid,
        min_seconds=task.min_sec,
        max_seconds=task.max_sec,
        require_full_horizon=bool(label_cfg.get("require_full_horizon", True)),
        min_future_observations=int(label_cfg.get("min_future_observations", 1)),
    )
    r_raw = labels.r_raw
    ...
    invalid_counts = {
        "invalid_current_mid": int(np.count_nonzero(invalid_reason & INVALID_CURRENT_MID)),
        "incomplete_horizon": int(np.count_nonzero(invalid_reason & INCOMPLETE_HORIZON)),
        "insufficient_future": int(np.count_nonzero(invalid_reason & INSUFFICIENT_FUTURE)),
        "nonfinite_return": int(np.count_nonzero(invalid_reason & NONFINITE_RETURN)),
    }
    ...
    try:
        np.savez(
            ...
            source_sha256=np.asarray(sha256_file(task.path)),
            t_sec=t_sec.astype(np.float64, copy=False),
            mid=mid.astype(np.float64, copy=False),
            r_raw=r_raw,
            is_valid=is_valid.astype(np.uint8),
            invalid_reason=invalid_reason,
            future_count=labels.future_count.astype(np.int32),
...
def require_current_raw_labels(labels):
    if (
        "label_contract_version" not in labels.files
        or int(labels["label_contract_version"]) != 2
        or labels["mid"].dtype != np.float64
    ):
        raise ValueError(
            "Raw labels lack the V5 unrounded-midpoint contract; rebuild Stage2 from CSV "
            "without --only-scale, then rebuild downstream artifacts."
        )


def worker_scale_from_raw(args: tuple) -> dict:
    """
    args = (raw_path, out_path, scale, clip_abs_scaled, binding)
    """
    raw_path, out_path, scale, clip_abs_scaled, binding = args
    z = None
    try:
        z = np.load(raw_path, allow_pickle=False)
        require_current_raw_labels(z)
        require_raw_definition(z, binding["target_contract"]["label_definition"])
        ...
        was_clipped = np.zeros(r_scaled.shape, dtype=np.uint8)
        if clip_abs_scaled is not None and float(clip_abs_scaled) > 0.0:
            limit = float(clip_abs_scaled)
            was_clipped[is_valid & (np.abs(r_scaled) > limit)] = 1
            np.clip(r_scaled, -limit, limit, out=r_scaled)
        r_scaled[~is_valid] = np.nan

        payload = {
            "label_contract_version": np.array(2, dtype=np.int16),
            "label_definition_sha256": z["label_definition_sha256"],
            "source_sha256": z["source_sha256"],
            "target_contract_sha256": np.asarray(binding["target_contract_sha256"]),
            "label_stats_sha256": np.asarray(binding["label_stats_sha256"]),
            "t_sec": z["t_sec"].astype(np.float64, copy=False),
            "mid": z["mid"].astype(np.float64, copy=False),
            "r_raw": r_raw,
            "r_scaled": r_scaled,
        ...
        if "invalid_reason" in z.files:
            payload["invalid_reason"] = z["invalid_reason"].astype(np.uint8, copy=False)
        if "future_count" in z.files:
            payload["future_count"] = z["future_count"].astype(np.int32, copy=False)
...
def main():
    ...
    ap.add_argument("--scale", type=float, default=None, help="Legacy check only: must equal label.fixed_scale")
    args = ap.parse_args()

    cfg = load_config(args.config_path)
    if args.scale is not None and args.scale != float(cfg.get("label", {}).get("fixed_scale", 1.0)):
        raise ValueError("set label.fixed_scale in the configuration so downstream contracts agree; --scale cannot override it")
    project_root = cfg["project"]["project_root"]
    ...
    logging.info(f"[Stage2] horizon={horizon_id} ({min_sec}-{max_sec}s), workers={num_workers}, only_scale={bool(args.only_scale)}")
    ...
    if not bool(args.only_scale):
        ...
        for g in good:
            ...
            index_rows.append({
                ...
                "n": g["n"],
                "valid": g["valid"],
                "invalid_counts": g.get("invalid_counts", {}),
            })
            if g["split"] == "train":
        ...
        for r in ok_rows:
            ...
            if not os.path.exists(raw_path):
                raise FileNotFoundError(f"missing raw labels: {raw_path}; rebuild Stage2 from CSV")
            if os.path.exists(raw_path):
                raw_paths.append(raw_path)
                with np.load(raw_path) as z:
                    try:
                        require_current_raw_labels(z)
                        require_raw_definition(z, label_definition(cfg), sha256_file(r["path"]))
                    except ValueError as exc:
                        logging.error("%s: %s", raw_path, exc)
                        sys.exit(4)
                    valid_count = int(z["is_valid"].astype(bool).sum())
                    n_rows = int(z["is_valid"].shape[0])
                index_rows.append({
                    "stock_code": sc,
                    "date": date,
                    "session": session,
                    "split": split,
                    "raw_label_path": raw_path,
                    "n": n_rows,
                    "valid": valid_count,
                })
        if not raw_paths:
    ...
    q100 = max(q100, q99)
    # scale = 1.0 / q90
    scale = float(args.scale if args.scale is not None else cfg.get("label", {}).get("fixed_scale", 1.0))
    if not np.isfinite(scale) or scale <= 0.0:
        logging.error(f"--scale must be finite and positive, got {scale}")
        sys.exit(8)

    # Clip only when explicitly configured. The threshold is derived from
    # train-only statistics and is now actually applied in the scale pass.
    clip_mode = str(cfg.get("label", {}).get("clip_mode", "q99_abs_train_only")).lower()
    if clip_mode in {"none", "off", "disabled"}:
        clip_abs_scaled = None
    elif clip_mode == "q99_abs_train_only":
        clip_abs_scaled = float(q99 * scale)
        max_clip = float(cfg["stage2"].get("max_clip_abs_scaled", 0.0))
        if max_clip > 0.0:
            clip_abs_scaled = min(max_clip, clip_abs_scaled)
        clip_abs_scaled = max(float(eps * scale), clip_abs_scaled)
    else:
        logging.error(f"Unsupported label.clip_mode={clip_mode!r}")
        sys.exit(9)
    ...
    stats_path = cfg["stage2"]["label_stats_path"].format(horizon_id=horizon_id)
    if not os.path.isabs(stats_path):
        stats_path = os.path.join(project_root, stats_path)
    stats["target_contract"] = target_spec(cfg, stats)
    stats["target_contract_sha256"] = json_sha256(stats["target_contract"])
    save_json(stats_path, stats)
    binding = binding_from_stats(cfg, stats, stats_path)
    logging.info(f"[Stage2] label stats saved: {stats_path}")
    clip_msg = "none" if clip_abs_scaled is None else f"{clip_abs_scaled:.6g}"
    logging.info(f"[Stage2] q90_abs={q90:.6g}, q99_abs={q99:.6g}, scale={scale:.6g}, clip_abs_scaled={clip_msg}")
    ...
    for item in index_rows:
        ...
        scale_jobs.append((raw_path, out_path, scale, clip_abs_scaled, binding))
    ...
    summary = {
        ...
        "label_contract": stats["label_contract"],
        "counts": {
            ...
            "rows": sum(int(x.get("n", 0)) for x in index_rows),
            "valid_rows": sum(int(x.get("valid", 0)) for x in index_rows),
            "invalid_reasons": {
                name: sum(int((x.get("invalid_counts") or {}).get(name, 0)) for x in index_rows)
                for name in [
                    "invalid_current_mid",
                    "incomplete_horizon",
                    "insufficient_future",
